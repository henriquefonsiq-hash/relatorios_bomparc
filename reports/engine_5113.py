"""
Engine para a máscara FOR-BPC-5113 — Relatório de Inspeção por Líquido Penetrante (LP).
"""
import io
import os
import docx
...
def build_5113(campos):
    app_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    template_path = os.path.join(app_dir, "template_5113_lp.docx")
    
    if not os.path.exists(template_path):
        template_path = os.path.join(app_dir, "template_reprovados.docx")

    item_atual = campos["itens_certificado"][0]
    certif     = f"{campos['numero_certificado']}-{item_atual}-LP"
